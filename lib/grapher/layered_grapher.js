LayeredGrapher = {
    ...
    insertPost: function(postArray, post) {
        ...
        if (target.replies.length > 0) {
            ...
            //posts above...
            while (targetId) {
                postArray.find(function(p) {return p.target == targetId}).sort(function(a, b) {
                ...
                targetId = postArray.find(function(p) {p._id == targetId}).target;
            };

            //...and all of a posts siblings.
            for (let id of target.replies) {
                let curPost = postArray.find(function(p) {p._id == id});
                if (curPost)
            ...
            for (let curPost of postsToShift) {
                ...
                postArray.find(function(p) {p.target == curPost._id}).forEach(function(child) {
