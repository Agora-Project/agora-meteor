...
LayeredGrapher = {
    ...
    assignRelativePositions: function(postCollection) {
        postCollection.find({}).fetch().forEach(function(node) {
            ...
            //Assign relative positions, compared to their target node.

            //First, find the current node, and iterate through it's children,
    ...
    insertPost: function(postCollection, post, direction) {
        ...
        //Adjust the chain of posts which need to be adjusted.
        if (shiftGraph) {
            //First, posts to the right of the new post
            this.shiftGraph(postCollection, x, 1).forEach((id) => {
                changedPosts.add(id);
            });

            //As well as those directly beneath it.
            this.adjustGraph(postCollection, {'position.x': x, 'position.y': {$lte: y}}, {$inc: {'position.x': 1}}).forEach((id) => {
                changedPosts.add(id);
            });
        }
