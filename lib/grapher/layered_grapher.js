...
LayeredGrapher = {
    ...
    layoutGraph: function(postCollection) {
        ...
        postCollection.find({}).fetch().forEach(function(node) {
            ...
            //Setup fields.
            postCollection.update({_id: node._id}, {$set: {layer: 0, position: {x: 0, y: 0}, subtreeWidth: 1, relativePos: 0, replyCount: postCollection.find({target: node._id}).count()}});
        ...
        for (let i = maxLayer - 1; i >= 0; i--) {
            ...
            for (let node of layer) {
                ...
                if (postCollection.find({target: node._id}).count() > 0) {
        ...
        postCollection.find({}).fetch().forEach(function(node) {
            ...
            postCollection.find({target: node._id}, {sort: {subtreeWidth: 1} }).forEach(function(below) {
    ...
    insertPost: function(postCollection, post) {
        ...
        if (!foundPlace) {
            if (target) {
                ...
                siblings = postCollection.find({target: post.target}).count();

                if (siblings) shiftGraph = true
            } else {
                ...
                siblings = postCollection.find({target: { "$exists" : false }}).count();
        ...
        if (shiftGraph) {
            ...
            if (postCollection.findOne({'position.x': x + siblings, 'position.y': {$lt: y + 2}}))
                postCollection.find({'position.x': {$gt: x}, 'position.y': {$gt: y}}).forEach(function(postToShift) {
            ...
            //As well as those beneath it.
            postCollection.find({'position.x': {$gt: x-1}, 'position.y': {$lt: y + 1}}).forEach(function(postToShift) {
        ...
        post.replyCount = postCollection.find({target: post._id}).count();
        postCollection.insert(post);
        postCollection.update({_id: post.target}, {$inc: {replyCount: 1}});
