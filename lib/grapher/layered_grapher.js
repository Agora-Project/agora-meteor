LayeredGrapher = {
    ...
    removePost: function(postArray, postID) {
        ...
        post.replies.forEach(function(reply) {
            self.removePost(postArray, reply);
        });
        ...
        if (target && target.edgesIn.length > 0) {
            let shifting = false;
            let postsToShift = [];
            let targetId = target.target;
            ...
            while (targetId) {
                postArray.filter(function(p) { return p.target == targetId}).sort(function(a,b) { return a.position.x - b.position.x}).forEach(function(p) {
                    ...
                    Posts.update({_id: p._id}, {$inc: {subtreeWidth: -1}});
            ...
            for (let id of target.edgesIn) {
                let p = postArray.find(function(p) { return p._id == id});
                if (p) {
                    if (shifting) {
                        if (p.position.x <= x) {
                            shifting = false;
        ...
        target.replies.pull(postId);
        postArray.pull(postId);
    }
}
