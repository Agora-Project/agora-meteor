...
LayeredGrapher = {
    ...
    insertPost: function(postCollection, post) {
        ...
        let x, y, shiftGraph = false, foundPlace = false;
        ...
        if (shiftGraph) {
            postCollection.find({'position.x': {$gt: shiftGraph}}).forEach(function(postToShift) {
                if (!postToShift)
                    return;
    ...
    removePost: function(postCollection, post) {
        ...
        if (postCollection.find({'position.x': post.position.x}).fetch().length <= 1 ||
            (postCollection.find({target: post.target}).fetch().length > 1 &&
