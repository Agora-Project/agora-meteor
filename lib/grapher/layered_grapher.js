...
LayeredGrapher = {
    ...
    insertPost: function(postCollection, post) {
        ...
        postCollection.find({inReplyTo: post._id}).forEach(function(reply) {

            if (!postCollection.findOne({'position.x': reply.position.x, 'position.y' : reply.position.y + 1}) && y <= reply.position.y + 1 && x >= reply.position.x) {
