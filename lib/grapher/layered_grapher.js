LayeredGrapher = {
    ...
    layoutGraph: function(postCollection) {
        ...
        let insertNode = function(node) {
            ...
            postCollection.update(node, {$set: {column: layer.length}});
            layer.push(node);
        }
            ...
            for (let node of layer) {
                if (node.replies.length > 0) {
                    for (let belowID of node.replies) {
                        ...
                        postCollection.update(node, {$inc: {subtreeWidth: below.subtreeWidth}});
                    }

                    postCollection.update(node, {$dec: {subtreeWidth: 1}});
                }
            }
        }
        ...
        postCollection.find({}).fetch().forEach(function(node) {
            ...
            for (let belowID of node.replies) {
                let below = postCollection.findOne({_id: belowID});
                postCollection.update(below, {$set: {relativePos: x}});
                x += below.subtreeWidth;
            }

            postCollection.update(node, {$set: {position: {y: -node.layer}}});
        });
        ...
        for (let layer of table) {
            for (let node of layer) {
                for (let nextID of node.replies) {
                    ...
                    postCollection.update(next, {$set: {"position.x": node.position.x + next.relativePos}});
                }
            }
        }
    },
    insertPost: function(postCollection, post) {
        ...
        if (target.replies.length > 0) {
            ...
            for (let post of postsToShift) {
                ...
                postCollection.update({_id: post._id}, {$set: {'defaultPosition.x': newColumn}});
