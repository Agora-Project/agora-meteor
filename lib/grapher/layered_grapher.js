...
LayeredGrapher = {
    ...
    insertPost: function(postCollection, post) {
        ...
        let x = target.position.x + siblings;
    ...
    removePost: function(postCollection, post) {
        ...
        if (!localPost) return {post: post, changedPosts: []};
        ...
        //Avoid shifting the graph if this post didn't have any siblings.
        if (shiftGraph && postCollection.find({target: post.target}).fetch().length < 2)
            shiftGraph = false;

        //Avoid shifting the graph is this post did have any children
        if (shiftGraph && postCollection.find({target: post._id}).fetch().length > 0)
            shiftGraph = false;
