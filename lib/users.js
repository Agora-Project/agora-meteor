...
if(Meteor.isServer) {
    ...
    Meteor.methods({
        ...
        userExists: function(preferredUsername) {
            return Meteor.users.findOne({'profile.preferredUsername': preferredUsername});
        }
    })

    Meteor.users.before.insert(function(userId, user) {
        if (!user.profile.name) user.profile.name = user.profile.preferredUsername;
